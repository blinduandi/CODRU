...
<div class="modal-content">
        ...
        <nav class="bg-dark">
          ...
          <header>
           ...
           <center style="margin-bottom:20px;"> <span style="color: white;font-size:20px;font-weight:800; ">Navigation</span><br></center>
            ...
            <a class="admilist"  onclick="edittitle(this.text, (document.getElementById('head_name').innerText))"><i class="fas fa-users"></i><span>Manage Users</span></a><br>
            ...
            <a class="admilist"  onclick="edittitle(this.text, (document.getElementById('head_name').innerText))"><i class="fal fa-browser"></i><span>Category</span></a><br>
  ...
  <span class="slider round"></span>
          ...
          <div class="flex-grid" id="div_Delivery" style="display:none;">
            ...
            $comand = DB::table('comand')->where('status',1)->get();
...
<?php 
$comand = DB::select('select * from comand where status = 2');
...
<?php 
$comand = DB::select('select * from comand where status = 3 or status = 4');
...
?>
          ...
          <div class="flex-grid" id="div_Category" style="display:none;">
            <div>
              <center>
              <a class="btn btn-secondary" onclick="editcat(this.text, (document.getElementById('head_cat').innerText))">Category</a>
              <a class="btn btn-secondary" onclick="editcat(this.text, (document.getElementById('head_cat').innerText))">Undercategory</a>
              <a class="btn btn-secondary" onclick="editcat(this.text, (document.getElementById('head_cat').innerText))">Specification Group</a>
              <a class="btn btn-secondary" onclick="editcat(this.text, (document.getElementById('head_cat').innerText))">Specification</a></center>
            </div>
<span style="display: none;" id="head_cat">Category</span>
            <div id="Category">
              <span style="font-size:30px;">Category</span>

              <button id="myBtn" class="btn btn-danger" style="float: right;padding:10px;" onclick="modal()">Add</button>
              <table width=100%>
                <td>Name</td>
                <td>Options</td>
                <?php
        ...
        foreach ($users as $user)
        {
          ...
          echo '<td>';
          //echo'  <button onclick="onclic(this)" class="btn btn-secondary myBtnunder" style="padding:8px;margin:5px;" value="'.$user->CATEGORY_ID.' '.$user->NAME.'">Undercategory</button>';
           echo '<form action="/admin/delcat" method="POST" style="display: inline;">';
            echo csrf_field();
            echo ' <input type="text" name="id" value="'.$user->CATEGORY_ID.'" hidden>';
           echo ' <input type="submit" value="Delete" class="btn btn-secondary" ></form>';

          // echo ' <a onclick="editcat(this)" class="btn btn-secondary" value="'.$user->CATEGORY_ID.'">Edit</a>     
         echo ' </td>';
          ...
          echo'</tr>';
            ...
            <div id="Undercategory"  style="display:none;">
              <span style="font-size:30px;" >Undercategory</span>

             <button id="myBtnunder" class="btn btn-danger" style="float: right;padding:10px;" onclick="modall()" >Add</button>
              <table width=100%>
                <td>Category</td>
                <td>Name</td>
                <td>Options</td>
        ...
        foreach ($users as $user)
        {
          ...
          //echo'  <button onclick="onclic(this)" class="btn btn-secondary myBtnunder" style="padding:8px;margin:5px;" value="'.$user->CATEGORY_ID.' '.$user->NAME.'">Undercategory</button>';
           echo '<form action="/admin/delcat" method="POST" style="display: inline;">';
            echo csrf_field();
            echo ' <input type="text" name="id" value="'.$user->UNDERCATEGORY_ID.''.$user->NAME.'" hidden>';
           echo ' <input type="submit" value="Delete" class="btn btn-secondary" ></form>';
         ...
         echo ' </td>';
          ...
          echo'</tr>';
            ...
            <div id="Specification_Group"  style="display:none;">
              <span style="font-size:30px;" >Specification Group</span>

              <button id="myBtnSg" class="btn btn-danger" style="float: right;padding:10px;" onclick="modalsg()">Add</button>
              <table width=100%>
                <td>Undercategory</td>
                <td>Name</td>
                <td>Options</td>
        ...
        {
          echo '<tr>';
           echo'<td>'.$user->UNDERCATEGORY_ID.'</td>';   
           echo'<td>'.$user->NAME.'</td>';             
          ...
          //echo'  <button onclick="onclic(this)" class="btn btn-secondary myBtnunder" style="padding:8px;margin:5px;" value="'.$user->CATEGORY_ID.' '.$user->NAME.'">Undercategory</button>';
           echo '<form action="/admin/delcat" method="POST" style="display: inline;">';
            echo csrf_field();
            echo ' <input type="text" name="id" value="'.$user->SPECIFICATION_GROUP_ID.''.$user->NAME.'" hidden>';
           echo ' <input type="submit" value="Delete" class="btn btn-secondary" ></form>';
         ...
         echo ' </td>';
          ...
          echo'</tr>';
            ...
            <div id="Specification"  style="display:none;">
              <span style="font-size:30px;" >Specification</span>

             <button id="myBtnS" class="btn btn-danger" style="float: right;padding:10px;" onclick="modals()">Add</button>
              <table width=100%>
          ...
          $users = DB::table('specification')->get();
        foreach ($users as $user)
        {
          echo '<tr>';
           echo'<td>'.$user->SPECIFICATION_GROUP_ID.'</td>';   
           echo'<td>'.$user->NAME.'</td>';             
          ...
          //echo'  <button onclick="onclic(this)" class="btn btn-secondary myBtnunder" style="padding:8px;margin:5px;" value="'.$user->CATEGORY_ID.' '.$user->NAME.'">Undercategory</button>';
           echo '<form action="/admin/delcat" method="POST" style="display: inline;">';
            echo csrf_field();
            echo ' <input type="text" name="id" value="'.$user->SPECIFICATION_ID.''.$user->NAME.'" hidden>';
           echo ' <input type="submit" value="Delete" class="btn btn-secondary" ></form>';
